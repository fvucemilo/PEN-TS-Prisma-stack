...
describe('Auth routes', () => {
  ...
  describe('POST /v1/auth/refresh-tokens', () => {
    ...
    test('should return 401 error if refresh token is signed using an invalid secret', async () => {
      ...
      const dbUserOne = (await prisma.user.findUnique({ where: { email: userOne.email } })) as User;
      const expires = moment().add(config.jwt.refreshExpirationDays, 'days');
      const refreshToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.REFRESH, 'invalidSecret');
      await tokenService.saveToken(refreshToken, dbUserOne.id, expires, TokenType.REFRESH);
  ...
  describe('POST /v1/auth/reset-password', () => {
    test('should return 204 and reset the password', async () => {
      ...
      const resetPasswordToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, dbUserOne.id, expires, TokenType.RESET_PASSWORD);
    ...
    test('should return 401 if reset password token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.resetPasswordExpirationMinutes, 'minutes');
      const resetPasswordToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.RESET_PASSWORD);
      await tokenService.saveToken(resetPasswordToken, dbUserOne.id, expires, TokenType.RESET_PASSWORD, true);
  ...
  describe('POST /v1/auth/verify-email', () => {
    test('should return 204 and verify the email', async () => {
      ...
      const verifyEmailToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.VERIFY_EMAIL);
    ...
    test('should return 401 if verify email token is blacklisted', async () => {
      ...
      const expires = moment().add(config.jwt.verifyEmailExpirationMinutes, 'minutes');
      const verifyEmailToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.VERIFY_EMAIL);
      await tokenService.saveToken(verifyEmailToken, dbUserOne.id, expires, TokenType.VERIFY_EMAIL, true);
...
describe('Auth middleware', () => {
  ...
  test('should call next with unauthorized error if access token is generated with an invalid secret', async () => {
    ...
    const dbUserOne = (await prisma.user.findUnique({ where: { email: userOne.email } })) as User;
    const expires = moment().add(config.jwt.accessExpirationMinutes, 'minutes');
    const accessToken = tokenService.generateToken(dbUserOne.id, expires, TokenType.ACCESS, 'invalidSecret');
